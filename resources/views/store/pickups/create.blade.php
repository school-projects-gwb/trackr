            ...
            <form method="POST" class="mt-5" action="{{ route('store.pickups.store') }}">
                ...
                @foreach($shipments as $shipment)
                    <div class="flex shipmentSelector pb-1" data-carrier="{{ $shipment->carrier_id }}">
                        <input type="checkbox" class="rounded-md border-gray-400 focus:border-gray-500 focus:ring-transparent mr-1" data-carrier="{{ $shipment->carrier_id }}" id="shipment-{{$shipment->id}}" name="shipment_id[]" value="{{ $shipment->id }}">
